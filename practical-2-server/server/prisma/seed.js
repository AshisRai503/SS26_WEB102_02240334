  ...
  for (const user of users) {
    for (let j = 1; j <= 5; j++) {
      const video = await prisma.video.create({
        data: {
          userId: user.id,
          caption: `This is video ${j} by ${user.username}`,
          videoUrl: `https://example.com/videos/${user.username}-video-${j}.mp4`,
          thumbnailUrl: `https://example.com/thumbnails/${user.username}-thumb-${j}.jpg`,
